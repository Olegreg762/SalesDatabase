...
#### Executive Summary
In this executive summary the design and implementation of the sales database will be explained. This database's intended purpose is to track and record the sales of each sales person and by each region the organization operates in. Originally the database was to be developed using Microsoft Access Database for the relational database. This was changed to MySQL as the availability of Windows based machines by the developers and maintainers of the database is too limited to support the use of Microsoft Access. The database will be developed in such a way that the capabilities and performance using MySQL will be equivalent to Microsoft Access. A demonstration dashboard has been created to support the use of the database, this dashboard is viewable by at from this [link](https://salesdatabase.streamlit.app/).
